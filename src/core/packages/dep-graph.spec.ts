...
describe('ZDepGraph', () => {
  ...
  it('reflects direct dependency', async () => {
    ...
    const packageB = await addPackage('packageB', {
      name: 'packageB',
  ...
  it('reflects transient dependencies', async () => {
    ...
    const packageC = await addPackage('packageC', {
      name: 'packageC',
  ...
  it('reflects recurrent dependencies', async () => {
    const packageA = await addPackage('packageA', {
      name: 'packageA',
  ...
  it('prefers dev and peer dependencies over dev ones', async () => {
    const packageA = await addPackage('packageA', { name: 'packageA' });
    const packageB1 = await addPackage('packageB1', {
      name: 'packageB1',
      devDependencies: {
        packageA: '*',
      },
    });
    const packageB2 = await addPackage('packageB2', {
      name: 'packageB2',
      devDependencies: {
        packageA: '*',
      },
    ...
    expect(dependantsOf(packageA)).toEqual(['packageB1', 'packageB2', 'packageC']);
    expect(dependenciesOf(packageB1)).toEqual(['packageA']);
    expect(dependenciesOf(packageB2)).toEqual(['packageA']);
    expect(dependenciesOf(packageC)).toEqual(['packageA', 'packageB2', 'packageB1']);
  });
  ...
  function dependenciesOf(target: ZPackage): readonly string[] {
    return [...target.depGraph().dependencies()].map(({ name }) => name);
  }

  function dependantsOf(target: ZPackage): readonly string[] {
